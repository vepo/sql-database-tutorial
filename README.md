...
```bash
docker-compose stop && docker-compose rm && docker volume ls -q | xargs docker volume rm
```

```bash
time docker exec  postgres-demo psql -U demo -d demo_db -c "SELECT * FROM employees;"
...
```bash
$ time docker exec  postgres-demo psql -U demo -d demo_db -c "SELECT id, name, department FROM employees;"   id    |     name      | department  
---------+---------------+-------------
